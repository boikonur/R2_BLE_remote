...
// { 0x0A, 0x13, 0x0D } =   , DeviceId.power (DID), PowerCommandIds.wake(CID)
...
enum class Flag : int
{
  ...
  // Packet(int device_id, int command_id, int flags = -1, int target_id = -1, int source_id = -1, int sequence = -1, std::vector<int> data = {})
  //   {
  //       this->flags = flags;
  //       if (flags == -1)
  //       {
...
// enum  R2LegAction(Enum):
//     stop = 0x00
//     three_legs = 0x01
//     two_legs = 0x02
//     waddle = 0x03

  enum UserIOCommandIds {
  allLEDs = 0x0e,
  allLEDsV21 = 0x1c,
  playAudioFile = 0x07,
  audioVolume = 0x08,
